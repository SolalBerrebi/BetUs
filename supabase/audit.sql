...
cross join lateral (values
  (1, 'Meilleur buteur',  tp.top_scorer,   tr.top_scorer,
      case when public.name_matches(tp.top_scorer, tr.top_scorer) then 12 else 0 end),
  (2, 'Meilleur passeur', tp.top_assister, tr.top_assister,
      case when public.name_matches(tp.top_assister, tr.top_assister) then 16 else 0 end),
  (3, 'Meilleur gardien', tp.best_keeper,  tr.best_keeper,
      case when public.name_matches(tp.best_keeper, tr.best_keeper) then 20 else 0 end),
  (4, 'Finale',
  ...
  (5, 'Équipe gagnante', tp.winner, tr.winner,
      case when tr.winner is not null and public.norm_name(tp.winner) = public.norm_name(tr.winner) then 30 else 0 end),
  (6, 'Meilleur joueur', tp.best_player, tr.best_player,
      case when public.name_matches(tp.best_player, tr.best_player) then 12 else 0 end)
